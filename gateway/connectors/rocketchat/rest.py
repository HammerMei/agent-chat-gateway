...
class RocketChatREST:
    ...
    async def post_message_returning_id(
        self,
        channel: str,
        text: str,
        tmid: str | None = None,
    ) -> str:
        """Post a message and return its ``_id`` for future updates or deletion.

        Identical to :meth:`post_message` but returns the new message's platform
        ID so the caller can later pass it to :meth:`update_message` or
        :meth:`delete_message`.

        Args:
            channel : Room ID or name.
            text    : Message body.
            tmid    : Thread root message ID.

        Returns:
            The ``_id`` of the newly created message.
        """
        if tmid:
            payload: dict = {"roomId": channel, "text": text, "tmid": tmid}
        else:
        ...
        if not result.get("success"):
            raise RuntimeError(
                f"post_message_returning_id failed: {result.get('error', result)}"
            )
        msg_id: str = result.get("message", {}).get("_id", "")
        if not msg_id:
            raise RuntimeError(
                f"post_message_returning_id: no _id in response: {result}"
            )
        logger.info(
            "Posted message %s to %s%s", msg_id, channel,
            f" (thread {tmid})" if tmid else "",
        )
        return msg_id
    ...
    async def update_message(
        ...
        """Update the text of an existing message in-place.

        Uses RC's ``chat.update`` endpoint.  Only the message author (or an
        admin) can update a message; this method is intended for bot-owned
        status placeholder messages.

        Args:
            room_id: Opaque RC room ID (required by ``chat.update``).
            msg_id : ``_id`` of the message to update.
            text   : New message body.
        """
        payload = {"roomId": room_id, "msgId": msg_id, "text": text}
        result = await self._request("POST", "chat.update", json_data=payload)
        if not result.get("success"):
            raise RuntimeError(
                f"update_message failed for msg {msg_id}: {result.get('error', result)}"
            )
        logger.debug("Updated message %s in room %s", msg_id, room_id)
    ...
    async def delete_message(
        ...
        Uses RC's ``chat.delete`` endpoint with ``asUser=true`` so the deletion
        is attributed to the bot account rather than an admin action.

        Args:
            room_id: Opaque RC room ID.
            msg_id : ``_id`` of the message to delete.
        """
        payload = {"roomId": room_id, "msgId": msg_id, "asUser": True}
        result = await self._request("POST", "chat.delete", json_data=payload)
        if not result.get("success"):
            raise RuntimeError(
                f"delete_message failed for msg {msg_id}: {result.get('error', result)}"
            )
        logger.debug("Deleted message %s from room %s", msg_id, room_id)

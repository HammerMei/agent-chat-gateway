...
- The operator is part of the string, so a rule allowing a redirect (`">>?\\s*'?(?-i:/tmp/).*"`) cannot also allow running `/tmp/evil.sh`. The target is shell-quoted when it needs to be (`> '/tmp/x y'`, `> ' /tmp/x'`), so a path with whitespace is visibly one word — hence the optional quote in the rule. A backslash-newline inside a target is not interpreted (its meaning depends on quoting this module does not lex); the raw text is matched, which fails closed. A literal target is shell-unquoted as a whole and `normpath`-ed, relative or absolute, so `..` cannot hide behind quotes or a prefix (`> /tmp/'..'/etc/passwd` and `> /tmp/../etc/passwd` both match as `> /etc/passwd`; `> logs/../../x` as `> ../x`). A target containing an expansion (`$VAR`, `${…}`, `$(…)`, backticks) or a glob metacharacter (`*`, `?`, `[`) has no knowable value before bash runs and is returned from that character onward with every literal prefix cut (`> ${HOME//root/../..}/etc/passwd`, `> **/../../x`), so no rule anchored on a path prefix can match it. Matching is lexical, like the file tools' `normpath`: symlinks are not resolved, so a directory rule confines the path *text*, not where a planted symlink points — use a private scratch directory if that matters. Input redirects (`< file`) are returned as well. Because params regexes are case-insensitive, a directory rule should pin the prefix with `(?-i:…)` as the shipped preset does.
- Not returned, because nothing is opened by the user's choice: descriptor duplications, moves and closes (`2>&1`, `>&2`, `3>&1-`, `3>&-`), sinks (`/dev/null`, `/dev/stdout`, `/dev/stderr`, `/dev/fd/N`) and a process-substitution target (`> >(cmd)` — the nested command is matched instead). Text the grammar cannot place (an `ERROR` node) is **not reconstructed**: a fragment carrying shell syntax is returned as its own string, which matches nothing, so the call fails closed. Known over-denials this causes, accepted: a heredoc with a partly quoted delimiter (`<< E"OF"`) and a here-string that follows another redirect (`2>/dev/null <<< hi`). A leading `~` is an expansion like `$` and is treated the same way.
- The `scratch-dir` preset in `config.example.yaml` is the one-line opt-in for owners who want `/tmp` writes: Write/Edit/MultiEdit (OpenCode's `edit`), the bash redirect, and OpenCode's `external_directory` ask (`/tmp/*`) together.

...
class TestTheSweepExpiresAStaleIdleRecord(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_a_pending_job_no_longer_holds_a_record_back(self):
        ...
        wake a deleted record". A job now records the room it targets and
        resurrects it through the ordinary rule path, so the premise is gone: a
        9am job on an expired room recreates its watcher at 9am, which is the
        feature working. The sweep has one condition fewer, and no longer
        disagrees with the operator's own `expire`.

        There is nothing left to inject here — the oracle it took is gone from
        the constructor, which the test below asserts directly.
        """
    ...
    def test_the_sweep_takes_no_job_oracle_at_all(self):
        """Gone from the signature, not merely unused: an argument a caller still
        passes is an argument it still believes protects something."""
        import inspect
